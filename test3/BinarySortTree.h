...
//	辅助函数:
    ...
    int BSTasl(BinTreeNode<ElemType> *t, int d)
    {
        d++;
        int a = d;
        if (t->leftChild)
            d += BSTasl(t->leftChild, a);
        if (t->rightChild)
            d += BSTasl(t->rightChild, a);
        return d;
    }
    ...
    void Delete1(BinTreeNode<ElemType> *&p);				// 删除p指向的结点
    void Delete2(BinTreeNode<ElemType> *&p);				// 删除p指向的结点
    void Delete3(BinTreeNode<ElemType> *&p);				// 删除p指向的结点
    void Delete4(BinTreeNode<ElemType> *&p);				// 删除p指向的结点
...
//  二叉排序树方法声明及重载编译系统默认方法声明:
    ...
    void ASL()                                              // 计算ASLsucc
    {
        cout << "该二叉排序树ASL值为：" << BSTasl(root, 0) * 1.0 / NodeCount() << endl;
    }
};
...
template <class ElemType>
void BinarySortTree<ElemType>::Delete1(BinTreeNode<ElemType> *&p)
// 操作结果: 删除p指向的结点
{
    ...
    if (p->leftChild == NULL && p->rightChild == NULL)	{	// p为叶结点
        ...
        if (tmpF->rightChild == tmpPtr)	// 删除tmpF的右孩子
            Delete1(tmpF->rightChild);
        else    // 删除tmpF的左孩子
            Delete1(tmpF->leftChild);
    }
}
