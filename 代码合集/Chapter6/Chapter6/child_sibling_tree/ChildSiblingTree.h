...
//  孩子兄弟表示树类方法的声明:
	...
	ChildSiblingTree<ElemType> &operator=(const ChildSiblingTree<ElemType>& copy);	
		// 重载赋值运算
	int GetDepthWithRecursive(ChildSiblingTreeNode<ElemType> *root);     // 递归获取树的深度 
	int GetDepthWithoutRecursive(ChildSiblingTreeNode<ElemType> *root);  // 非递归求树的深度 
};

/// @author 20123101 李昀哲  2022.3.8
template <class ElemType>
int ChildSiblingTree<ElemType>::GetDepthWithRecursive(ChildSiblingTreeNode<ElemType> *root)
// 以孩子-兄弟表示法作为树的存储结构，递归算法求深度 
{
	if (root == NULL)                                             // 该结点为空时，深度为0 
		return 0;
	if (root != NULL)                                             // 该结点不空 
	{
		int h_first_child = 0, h_next_sibling = 0;                // 定义局部变量，分别表示first_child子树高度、next_sibling子树高度           
		h_first_child = GetDepthWithRecursive(root->firstChild);  // 递归求first_child子树深度（first_child若无子树，直接返回0，继续检查next_sibling） 
		h_next_sibling = GetDepthWithRecursive(root->nextSibling);// 递归求next_sibling子树深度（first_child若无子树，再检查next_sibling，
																  // 直到没有next_sibling才会返回） 
		if (h_first_child + 1 > h_next_sibling)                   // 由于上述原因，使得first_child子树需要+1弥补first_child直接返回0的情况 
			return  h_first_child + 1;
		else
			return h_next_sibling;
	}	
} 

// @author 20123101 李昀哲  2022.3.9
template <class ElemType>
int ChildSiblingTree<ElemType>::GetDepthWithoutRecursive(ChildSiblingTreeNode<ElemType> *root)
{
	int depth = 1;														// 定义初始深度为1																	
	bool is_first_existed = false, debug = false;						// 定义每层第一个结点是否出现以及定义调试变量 
	
	LinkQueue<ChildSiblingTreeNode<ElemType> *> assist_queue;           // 定义辅助队列，用于层次遍历 
	assist_queue.EnQueue(root);											// 根节点入队 
	ChildSiblingTreeNode<ElemType> *p = root, *first_node_in_one_level; // 定义局部变量，操作结点 
	while(p->firstChild || p->nextSibling)                              // 循环终止条件为当前结点无孩子和兄弟，即：最终的叶子节点 
	{
		assist_queue.DelQueue(p);                                       // 队首元素出队 
		if (debug)														// 显示调试信息 
			std::cout << "DelQueue: " << p->data << std::endl;
		for(ChildSiblingTreeNode<ElemType> *temp = p->firstChild; temp!=NULL; temp = temp->nextSibling)  // 当前结点的所有孩子从左至右入队 
		{
			assist_queue.EnQueue(temp);
			if (debug)
				std::cout << "EnQueue: " << temp->data << std::endl;
		}
		
		if(is_first_existed == false)                                   // 遍历到每一层第一个结点时，记录之。 
		{
			first_node_in_one_level = p->firstChild;
			is_first_existed = true;
		}
		if(p->nextSibling)                                              // 遍历每一层所有结点，故移至其next_sibling 
		{
			p = p->nextSibling;	
			continue;
		}
		depth ++;														// 否则表示这一层结束，移至下一层 

		if(first_node_in_one_level)                                     // 存在第一个结点，继续下移。 
		{
			p = first_node_in_one_level;
			is_first_existed = false; 
			continue;
		}
		break; 
	}
